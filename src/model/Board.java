...
public final class Board extends Observable
{
  ...
  /*
   * ***********************************************************************************
   * This is the basic score added each time you clear a block.
   */
  /** The score that's added when each line is cleared. */
  private static final int SCORE_BASIC = 100;
  
   /*
    * This is the multiplier that is multiplied by the level
    * each time you get a multiple of 500 points.
    */
  /** The score that's added when the score is a multiple of 500. */
  private static final int SCORE_LEVEL = 500;
  
  /** The score that's added when each line is cleared. */
  private static final int SCORE_EXTRA = 10000;
  /*
   * If you reach this score you get a big bonus to your score.
   * *************************************************************************************
   */
  ...
  /*
   * ***********************************************************************************
   * Added to keep track of my score for the board panel.
   */
  /**
   * The sore of the game.
   */
  private int my_score;  
  
  /*
   * Added to keep track of the lines that are cleared.
   */
  /**
   * Keeps track of the lines cleared.
   */
  ...
  /*
   * Added to keep track of the level for my board panel.
   * ***********************************************************************************
   */
                      ...
                      final List<MutablePiece> the_pieces)
  {
     ...
     * ***********************************************************************************
     * Added to keep track of my score for the board panel.
     * Added the my_lines to keep track of the lines that are cleared.
     */
    my_score = 0; 
    my_lines = 0;
    my_level = 1;  
  ...
  /*
   * **********************************************************************************
   * Changed this method to public so my board panel can get the current piece.
   * **********************************************************************************
   */
  {
  ...
  private void clearLines()
  {
    for (int i = my_blocks.size() - 1; i >= 0; i--)
    {
      ...
      if (clear)
      {
        ...
        /*
         * ********************************************************************************
         * Added this method call to the method below that sets the score and level.
         */
        setScoreLevel();
        /*
         * ********************************************************************************
  ...
  /**
   * Sets the level and the score of the game.
   * Added to the board class.
   */
  private void setScoreLevel() {
    /*
     * ***********************************************************************************
     * I added to the score here to keep track of the score in the game.
     * I then also updated the level if the score is a multiple of 500.
     * The score is increased by 500 if you level up.
     * Then if the score is is over 10,000 then you get extra points added.
     */
    my_score += SCORE_BASIC;
    my_lines += 1;
    if ((my_score % SCORE_LEVEL) == 0) {
      my_score += SCORE_LEVEL * my_level;
      my_level++;
    }
    if ((my_score / SCORE_EXTRA) >= 1 && my_score <= (SCORE_EXTRA + SCORE_LEVEL)) {
      my_level++;
      my_score += SCORE_EXTRA;
    }
  }
  /*
   * End of my addition here. **********************************************************
   */
  
  /*
   * *************************************************************************************
   * added to keep track of the score by the game.
   */
  /**
   * This returns the score of the game.
   * @return my_score - the score.
   */
  public int getScore() {
    return my_score;
  }
  ...
  /**
   * Returns the lines that have been cleared during the game.
   * @return my_lines - the lines cleared.
   */
  public int getLines() {
    /*
     * added to keep track of my cleared lines by the game.
     */
    return my_lines;
  }
  
  /**
   * This returns the level the game is currently at.
   * @return my_level - the level.
   */
  public int getLevel() {
    return my_level;
  }
